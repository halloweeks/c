...
void SendChatMessage(int sock, const char *message) {
	uint8_t buffer[BUFFER_SIZE];
	size_t  offset = 0;
	int msg_len = strlen(message) + 1;
	
	if (msg_len >= 65535) return;
	
	uint16_t packet_type = _MSG_RESP_CHATMESSAGE;
	uint16_t packet_size = 0;
	
	memcpy(buffer + offset, &packet_size, 2);  offset += 2;
	memcpy(buffer + offset, &packet_type, 2);  offset += 2;
	memcpy(buffer + offset, &msg_len, 2);      offset += 2;
	memcpy(buffer + offset, message, msg_len); offset += msg_len;
	memcpy(buffer, &offset, 2); // rewrite packet size 
...
void *connection_handler(void *sock_fd) {
	...
	/*
	uint16_t packet_type = _MSG_RESP_ACTIVE;
	uint16_t packet_size = 104;
	uint8_t data[100] = {0x79};
	uint8_t data2[200] = {0};
	memcpy(data2, "halloweeks", 10);
	
	
	uint8_t buffer[BUFFER_SIZE];
	uint32_t offset = 0;
	
	memcpy(buffer + offset, &packet_size, 2);
	offset += 2;
	memcpy(buffer + offset, &packet_type, 2);
	offset += 2;
	memcpy(buffer + offset, data, sizeof(data));
	offset += sizeof(data);
	
	
	packet_type = _MSG_RESP_CHATMESSAGE;
	packet_size = 204;
	...
	memcpy(buffer + offset, &packet_type, 2);
	offset += 2;
	memcpy(buffer + offset, data2, sizeof(data2));
	offset += sizeof(data2);
	
	
	
	if (send(conn_id, buffer, offset, 0) > 0) {
		printf("[SEND] %u bytes\n", offset);
	} else {
		printf("[WARNING][SEND] %u bytes\n", offset);
	}
	*/
	
	for (int i = 0; i < 10; i++) {
    SendChatMessage(conn_id, "halloweeks");
    sleep(1);
    }
    /*
