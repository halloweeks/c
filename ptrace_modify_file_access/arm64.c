...
long ptrace_write(pid_t pid, void *addr, void *data, size_t len) {
    ...
    // Write data in chunks of sizeof(long) bytes (8 bytes on ARM64)
    for (i = 0; i < len; i += sizeof(long)) {
        long data_in_long = data_ptr[i / sizeof(long)];
        // Write chunk to the target process
        long result = ptrace(PTRACE_POKEDATA, pid, addr + i, data_in_long);
        if (result == -1) {
            perror("ptrace write failed");
            return -1;
        }
    }
...
void trace_syscalls(pid_t target_pid) {
	...
	long syscall = -1;
	char filename[4096]; // 4KB
	long namesize = -1;
	
	char name[1024] = "/sdcard/pubgmobile/1.txt\0";
	...
	while (1) {
		...
		if (syscall == __NR_openat) {
			...
			namesize = read_string(target_pid, regs.regs[1], filename, 1024);
			// printf("file name: %s\n", filename);
