...
void trace_syscalls(pid_t target_pid) {
	...
	while (1) {
		...
		if (syscall == __NR_openat) {
			...
			if (strstr(filename, "good.txt") != NULL) {
				printf("file: %s\n", filename);
				ptrace_write(target_pid, regs.regs[1], "/storage/emulated/0/pubgmobile/bad.txt\0", 40);
		...
		if (syscall == __NR_read) {
			if (strstr(__symlink(target_pid, regs.regs[0]), "bad.txt") != NULL) {
